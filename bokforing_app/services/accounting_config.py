...
# Globala variabler som kommer att laddas från databasen
KONTOPLAN = {
    # Dina ursprungliga poster (behållna och integrerade)
    '1613': 'Lön',
    '1630': 'Skatteverket',
    '1680': 'Lån',
    ...
    '3041': 'Försäljning',
    '4010': 'Inköp av varor och material',  # <-- Ditt viktiga konto
    ...
    # Full BAS 2019 (extraherad från PDF, sorterad numeriskt)
    '1010': 'Utvecklingsutgifter [Ej K2]',
    '1011': 'Balanserade utgifter för forskning och utveckling [Ej K2]',
    ...
    '7034': 'Löner kollektivanställda 16,36 %',
    '7035': 'Löner till kollektivanställda 6,15 %',
...
def load_accounting_config():
    """
    Laddar kontoplanen och associationskartan från databasen.
    Denna funktion bör anropas inom en applikationskontext.
    """
    global KONTOPLAN, ASSOCIATION_MAP

    # Populate Konto table if it's empty
    if Konto.query.count() == 0:
        current_app.logger.info("Konto table is empty. Populating with initial data.")
        for konto_nr, beskrivning in KONTOPLAN.items():
            konto = Konto(konto_nr=konto_nr, beskrivning=beskrivning)
            db.session.add(konto)
        db.session.commit()
        current_app.logger.info(f"Populated Konto table with {len(KONTOPLAN)} entries.")

    # Ladda kontoplan
    konton = Konto.query.all()
    KONTOPLAN = {konto.konto_nr: konto.beskrivning for konto in konton}

    # Ladda associationskarta
    associations = Association.query.all()
    ASSOCIATION_MAP = {assoc.keyword: assoc.konto_nr for assoc in associations}

    current_app.logger.info("Accounting config loaded from DB.")

# Initial laddning (kan behöva anropas explicit vid appstart)
